    ...
    Private Function uiTrnBanktransfer_Print() As Boolean
        ...
        Select Case Me.ftabMain.SelectedTab.Name
            Case ftabMain_List.Name
                Try
                    ...
                    For t = 0 To Me.DgvTrnBanktransfer.SelectedRows.Count - 1
                        ...
                        If _MANUAL Then
                            ...
                            'Me.DgvTrnBanktransfer.SelectedRows(t).Cells("rekanan_id").Value

                            'Me.tbl_artist.Select(Me.DgvTrnBanktransfer.SelectedRows(t).Cells("rekanan_id").Value, "code")  'Me.DgvTrnBanktransfer.SelectedRows(t).Cells("rekanan_id").Value
                ...
                'Case ftabMain_Data.Name

                '    Try
                '        dbConn.Open()
                '        banktransfer_id = Me.obj_Banktransfer_id.Text

                '        dbDA_header.SelectCommand.Parameters("@banktransfer_id").Value = banktransfer_id

                '        'terbilang = clsUtil.Terbilang(Val(Me.obj_Banktransfer_idr.Text) - Val(Me.obj_Banktransfer_bi_idr.Text))
                '        terbilang = clsUtil.Terbilang(tbl_TrnBanktransfer_Temp.Rows(0)("banktransfer_idr") - tbl_TrnBanktransfer_Temp.Rows(0)("banktransfer_bi_idr"))
                '        dbDA_header.SelectCommand.Parameters("@terbilang").Value = "# " + Trim(terbilang) + " rupiah #"

                '        strMessage = String.Empty
                '        If _PEMBAYARAN_HONOR Then
                '            Dim strEpsTemp As String = String.Empty
                '            strEpsTemp = IIf(Mid(obj_Episode.Text, 1, 1) = "#", Mid(obj_Episode.Text, 2, obj_Episode.Text.Length - 1), obj_Episode.Text)
                '            strMessage = String.Format("Pbyrn honor {0} u/ program {1} #{2}", obj_Rekanan_id.Text.ToUpper, obj_ProjectID.Text, strEpsTemp)
                '        End If
                '        If _PEMBAYARAN_TAGIHAN Then
                '            strMessage = String.Format("Pbyrn tagihan u/ no invoice : {0}", obj_Invoice.Text)
                '        End If
                '        If _MANUAL Then
                '            strMessage = obj_Banktransfer_message.Text.Trim
                '        End If
                '        dbDA_header.SelectCommand.Parameters("@message").Value = strMessage
                '        dbDA_header.SelectCommand.Parameters("@paymenttype_id").Value = Me.obj_Banktransfer_pembayaran.SelectedValue
