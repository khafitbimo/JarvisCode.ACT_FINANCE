'modified by Ari Prasasti
'Trans Tv 05 July 2012
Public Class dlgTrnJurnal_PV_List_TransferRekening
...
#Region " Constructor & Default Function"
    ...
    Public Sub New(ByVal dsn As String, ByVal channel_id As String, ByVal dgvCount As Integer, _
            ByVal isPosted As Boolean, ByVal tbl_TrnBankTransferTemps As DataTable, ByVal currency_id As String, _
            ByVal amountForeign As Decimal, ByVal rate As Decimal, ByVal amountIdr As Decimal, ByVal paymentType As String, _
            ByVal tbl_MstSlipFormat_temps As DataTable, ByVal tbl_MstCurrency_temps As DataTable, _
            ByVal tbl_MstPaymentType_temps As DataTable, ByVal tbl_MstPurposeFund_temps As DataTable, _
            ByVal tbl_project As DataTable, ByVal jurnal_id As String, ByVal jurnaldetil_line As Integer, Optional ByVal tbl_listRekening As DataTable = Nothing, Optional ByVal channel As String = "NTV", _
            Optional ByVal username As String = "", Optional ByVal rekanan As Integer = 0)
        _
...
#End Region
    ...
    Private Sub dlgTrnJurnal_PV_List_TransferRekening_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Me.tbl_listRekening.Rows.Count > 0 Then
            Me.Btn_Add.Visible = True
            Me.btn_listRekening.Visible = False
        Else
            Me.Btn_Add.Visible = False
            Me.btn_listRekening.Visible = True
        End If
            ...
            If Me.mIsPosted = False Then
                ...
                If retTbl IsNot Nothing Then
                    If retTbl.Rows.Count <> 0 Then
                        ...
                        'row.Item("jurnal_id") = retTbl.Rows(0).Item("jurnal_id")
                        'row.Item("jurnaldetil_line") = retTbl.Rows(0).Item("jurnaldetil_line")
                        'row.Item("rekanan_id") = retTbl.Rows(0).Item("rekanan_id")
                        'row.Item("rekananbank_line") = retTbl.Rows(0).Item("rekananbank_line")
                        'row.Item("currency_id") = retTbl.Rows(0).Item("currency_id")
                        'row.Item("paymenttype_id") = retTbl.Rows(0).Item("paymenttype_id")
                        'If row.Item("paymenttype_id") = "4" Then
                        ...
                        'row.Item("jurnalbilyet_receiverekening") = retTbl.Rows(0).Item("banktransfer_rekening")
                        'row.Item("jurnalbilyet_receivebank") = retTbl.Rows(0).Item("banktransfer_pembayaranrek")
                        'row.Item("jurnalbilyet_receiveaccountname") = Me.tbl_listRekening.Rows(0).Item("bank_receivedby")
                        'row.Item("project_id") = Me.tbl_listRekening.Rows(0).Item("show_id")
                        'row.Item("banktransfer_episode") = Me.tbl_listRekening.Rows(0).Item("episode")
    ...
    Private Sub btn_listRekening_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_listRekening.Click
        Dim oDataFiller As New clsDataFiller(Me.DSN)
        If mDgvCount > 0 Then
            If Me.mIsPosted = False Then
                Dim retTbl As DataTable = New DataTable
                Dim dlg As New dlgTrnJurnal_PV_List_Rekening(Me.DSN, Me.rekanan)
                Dim row As DataRow
