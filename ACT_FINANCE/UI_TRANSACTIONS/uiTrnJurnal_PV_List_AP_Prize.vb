...
Public Class uiTrnJurnal_PV_List_AP_Prize
    Implements ILocking
    ...
    Private ConstMyJurnalSource As String = clsSource.PVListAPPrize 'clsSource.PVListTax 
...
#Region " Layout & Init UI "
    ...
    Private Function FormatDgvTrnBanktransfer(ByRef objDgv As System.Windows.Forms.DataGridView) As Boolean
        ...
        Dim cInitial As New System.Windows.Forms.DataGridViewTextBoxColumn
        ...
        cInitial.Name = "Initial"
        cInitial.HeaderText = "Initial"
        cInitial.DataPropertyName = "Initial"
        cInitial.Width = 70
        cInitial.Visible = False
        cInitial.ReadOnly = True
        cInitial.DefaultCellStyle.BackColor = Color.Gainsboro
        ...
        cRekening_debit, cBank_debit, cAccount_bank_debit, cInitial})
        objDgv.AutoGenerateColumns = False
        objDgv.AllowUserToAddRows = False
        objDgv.AllowUserToDeleteRows = False
        objDgv.ReadOnly = True
...
#Region " Dialoged Control "
    Private Function DialogOpen_Reference_List_AP() As Boolean
        Dim dlg As dlgTrnJurnal_PV_Select_AP_Prize = New dlgTrnJurnal_PV_Select_AP_Prize(Me.DSN, Me.tbl_MstRekananGrid.Copy, Me.tbl_MstAccGrid.Copy, Me._CHANNEL)
        ...
        If retObj IsNot Nothing Then
            ...
            For k = 0 To tblJDetil.Rows.Count - 1
                ...
                If tblJDetil.Rows(k).Item("acc_id").ToString = "2140700" Then 'Account GR/IR - Pph21 - Hadiah
                    row_kredit.Item("acc_id") = "2149008" 'Clearing 2 - PPh Psl 21 - Hadiah
...
#Region " User Defined Function "
    ...
    Private Function uiTrnJurnal_PV_List_AP_OpenRowTransfer(ByVal channel_id As String, ByVal jurnal_id As String, ByVal dbConn As OleDb.OleDbConnection) As Boolean
        ...
        dbCmd = New OleDb.OleDbCommand("cp_TrnBanktransfer_Select_Prize", dbConn)
        dbCmd.Parameters.Add("@channel_id", Data.OleDb.OleDbType.VarChar)
                ...
                Case Me.DgvTrnJurnaldetil_Credit.Columns("select_received").Index 'Button transfer
                    ...
                    If CType(acc_id, Integer) <> 0 Then
                        ...
                        If acc_id = 111 Then '100 Then
                            ...
                            Dim dlg As dlgTrnJurnal_PV_List_TransferRekeningWinner = New dlgTrnJurnal_PV_List_TransferRekeningWinner(Me.DSN, _
                                                Me._CHANNEL, _
                                                Me.DgvTrnJurnal.Rows.Count, _
                                                Me.obj_Jurnal_isposted.Checked, _
                                                Me.tbl_TrnBankTransfer.Copy, _
                                                ...
                                                Me.tbl_MstSlipFormat.Copy, _
                                                Me.tbl_MstCurrency.Copy, _
                                                Me.tbl_MstPaymentTypeGrid.Copy, _
                                                Me.tbl_MstPurposeFund.Copy, _
                                                Me.tbl_MstShow.Copy, _
                                                Me.DgvTrnJurnaldetil_Credit.Rows(e.RowIndex).Cells("jurnal_id").Value, _
                                                Me.DgvTrnJurnaldetil_Credit.Rows(e.RowIndex).Cells("jurnaldetil_line").Value, _
                                                "RekArtisOn", _
                                                "RekWinnerOn", _
                                                ...
                                                Me._CHANNEL, _
                                                Me.UserName, _
                                                Me.obj_Rekanan_id.SelectedValue, _
                                                Me.tbl_TrnJurnaldetil_Debit
                                                )
                            'Me.tbl_TrnContract
                            Dim retObj As Object
                            Dim retData As Collection
...
#End Region
    ...
    Private Sub btnToExcel_Click(sender As Object, e As EventArgs) Handles btnToExcel.Click
        Dim objTblBankTransfer As DataTable = CType(Me.dgvTrnBankTransfer.DataSource, DataTable)
        ...
        shXL.Cells(6, 2).Value = "No. "
        shXL.Cells(8, 2).Value = "Kepada Yth"
        shXL.Cells(9, 2).Value = "PT. Bank Danamon Indonesia"
        shXL.Cells(10, 2).Value = "The East Building"
        shXL.Cells(11, 2).Value = "Jakarta"
        shXL.Cells(13, 2).Value = "Dengan Hormat"
        shXL.Cells(15, 2).Value = "Dengan ini mohon untuk mendebet rekening kami"
        shXL.Cells(17, 2).Value = "Nama : PT. Net Mediatama Televisi"
        shXL.Cells(18, 2).Value = "No. Rekening : 3569-4853-64"
        shXL.Cells(19, 2).Value = "Jumlah : Rp. " + Format(objTblBankTransfer.Compute("sum(banktransfer_idr)", ""), "N0")
        shXL.Cells(21, 2).Value = "Dan mohon dana tersebut ditransfer ke rekening berikut :"

        shXL.Cells(23, 2).Value = "No."
        shXL.Cells(23, 3).Value = "Nama"
        shXL.Cells(23, 4).Value = "Bank Rekening"
        shXL.Cells(23, 5).Value = "No. Rekening"
        shXL.Cells(23, 6).Value = "Jumlah (Rp.)"
        shXL.Cells(23, 7).Value = "Keterangan"

        Dim idx As Integer = 24

        For i As Integer = 0 To Me.dgvTrnBankTransfer.Rows.Count - 1
            shXL.Cells(idx + i, 2).Value = (i + 1).ToString
            shXL.Cells(idx + i, 3).Value = Me.dgvTrnBankTransfer.Rows(i).Cells("Receive By").Value.ToString
            shXL.Cells(idx + i, 4).Value = Me.dgvTrnBankTransfer.Rows(i).Cells("Receive Bank").Value.ToString
            shXL.Cells(idx + i, 5).Value = Me.dgvTrnBankTransfer.Rows(i).Cells("Receive Rekening").Value.ToString
            shXL.Cells(idx + i, 6).Value = Format(Me.dgvTrnBankTransfer.Rows(i).Cells("banktransfer_idr").Value, "N0")
            shXL.Cells(idx + i, 7).Value = Me.obj_Jurnal_id.Text.ToString() + " " + Me.dgvTrnBankTransfer.Rows(i).Cells("Initial").Value.ToString + " - " + Me.dgvTrnBankTransfer.Rows(i).Cells("Rekanan Name Report").Value.ToString
        Next

        idx = idx + (Me.dgvTrnBankTransfer.Rows.Count)
        shXL.Cells(idx, 3).Value = "Total : "
        shXL.Cells(idx, 3).Font.Bold = True
        shXL.Cells(idx, 6).Value = Format(objTblBankTransfer.Compute("sum(banktransfer_idr)", ""), "N0")
        shXL.Cells(idx, 6).Font.Bold = True

        shXL.Cells(idx + 2, 2).Value = "Demikian permohonan kami atas kerjasamanya kami ucapkan terima kasih"
        shXL.Cells(idx + 4, 2).Value = "Hormat Kami,"
        shXL.Cells(idx + 8, 2).Value = "Kurnia"
        shXL.Cells(idx + 8, 2).Font.Underline = True
        shXL.Cells(idx + 9, 2).Value = "Direktur"

        '' Format A1:D1 as bold, vertical alignment = center.

        With shXL.Range("B23", "F23")
            .Font.Bold = True
            .VerticalAlignment = Excel.XlVAlign.xlVAlignCenter
            .Borders.LineStyle = Nothing
        End With
