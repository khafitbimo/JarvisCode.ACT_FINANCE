...
Public Class uiTrnCirculationVoucher
    ...
    Private _AppDirFromContext As Boolean = False
    Private _CancelCellAppDir As Boolean = False
    'tambahan
    Private tbl_TrnCirculationvoucherdetil_all As DataTable = clsDataset.CreateTblTrnCirculationvoucherdetil()
    Private tbl_trnCirculationDetil_ExportExcel As New DataTable
...
#Region " Window Parameter "
    ...
    Private _EmailPIC As String = "(Anwar / Tika di ext. 7631 / Pramesthi (Kasir) di ext. 7624)."
...
#Region " Additional Overrides "
    ...
    Private Sub btnUnApproved_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUnApproved.Click
        'If paidIsApproved() Then
        '    MsgBox("some detail data has been paid Approved")
        '    Exit Sub
        'End If
        If Me.DgvTrnCirculationvoucher.Rows.Count > 0 Then
            DataIsApproved(0)
    ...
    Private Sub btnExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExcel.Click
        ...
        If Me.tbl_trnCirculationDetil_ExportExcel.Rows.Count <= 0 Then
            MessageBox.Show("Cannot export empty table.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
            If Me.tbl_trnCirculationDetil_ExportExcel.Rows.Count - 1 > 65534 Then
            ...
            Else
                ...
                Try
                    ...
                    xlWorksheet.Cells(1, 2).Value = "Order ID"
                    xlWorksheet.Cells(1, 3).Value = "Tanda Terima ID"
                    xlWorksheet.Cells(1, 4).Value = "Invoice"
                    ...
                    'tambahan
                    'xlWorksheet.Cells(1, 8).Value = "Foreign"

                    For Each row As System.Data.DataRow In Me.tbl_trnCirculationDetil_ExportExcel.Rows
                        RowIndex = RowIndex + 1
                        ColumnIndex = 0
                        ...
                        If Mid(row.Item("order_id").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 2).Value = "'" & row.Item("order_id").ToString Else xlWorksheet.Cells(RowIndex + 1, 2).Value = row.Item("order_id").ToString

                        'tambahan
                        If Mid(row.Item("tandaterima_id").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 3).Value = "'" & row.Item("tandaterima_id").ToString Else xlWorksheet.Cells(RowIndex + 1, 3).Value = row.Item("tandaterima_id").ToString

                        If Mid(row.Item("tandaterima_invoiceno").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 4).Value = "'" & row.Item("tandaterima_invoiceno").ToString Else xlWorksheet.Cells(RowIndex + 1, 4).Value = row.Item("tandaterima_invoiceno").ToString

                        If Mid(row.Item("tandaterima_duedate").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 5).Value = "'" & IIf(IsDBNull(row.Item("tandaterima_duedate")), "", Microsoft.VisualBasic.Left(row.Item("tandaterima_duedate").ToString(), 10)) Else xlWorksheet.Cells(RowIndex + 1, 5).Value = IIf(IsDBNull(row.Item("tandaterima_duedate")), "", Microsoft.VisualBasic.Left(row.Item("tandaterima_duedate").ToString(), 10))
                        'end tambahan

                        If Mid(row.Item("payment").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 6).Value = "'" & row.Item("payment").ToString Else xlWorksheet.Cells(RowIndex + 1, 6).Value = row.Item("payment").ToString

                        If Mid(row.Item("circulationdetil_bilyet").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 7).Value = "'" & row.Item("circulationdetil_bilyet").ToString Else xlWorksheet.Cells(RowIndex + 1, 7).Value = row.Item("circulationdetil_bilyet").ToString

                        If Mid(row.Item("rekanan_name").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 8).Value = "'" & row.Item("rekanan_name").ToString Else xlWorksheet.Cells(RowIndex + 1, 8).Value = row.Item("rekanan_name").ToString

                        If Mid(row.Item("circulationdetil_descr").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 9).Value = "'" & row.Item("circulationdetil_descr").ToString Else xlWorksheet.Cells(RowIndex + 1, 9).Value = row.Item("circulationdetil_descr").ToString

                        If row.Item("currency_id") = 1 Then
                            If Mid(row.Item("reftandaterima_amountidr").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 10).Value = "'" & row.Item("reftandaterima_amountidr") Else xlWorksheet.Cells(RowIndex + 1, 10).Value = row.Item("reftandaterima_amountidr")
                        Else
                            If Mid(row.Item("reftandaterima_amountforeign").ToString, 1, 1) = "=" Then xlWorksheet.Cells(RowIndex + 1, 10).Value = "'" & row.Item("reftandaterima_amountforeign") Else xlWorksheet.Cells(RowIndex + 1, 11).Value = row.Item("reftandaterima_amountforeign")
                        End If

                        lblProgress.Text = "Exporting " & RowIndex & " of " & Me.tbl_trnCirculationDetil_ExportExcel.Rows.Count & " records"
                        lblProgress.Refresh()
                        ProgressBar1.Value = CInt(100 * (RowIndex / Me.tbl_trnCirculationDetil_ExportExcel.Rows.Count))
    ...
    Private Sub btn_CheckAllAppDir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_CheckAllAppDir.Click, btn_UnCheckAllAppDir.Click
        Dim checkingEmail = Function(ByRef rekanans As List(Of dlgTrnCirculationRekanan.CirculationRekananItem)) As Boolean
                                Dim db As New DataClassesFrmDataContext(Me.DSNLinq)

                                db.OpenConnectionWithRole()

                                Try
                                    For i As Integer = 0 To Me.DgvTrnCirculationvoucherdetil.Rows.Count - 1
                                        Dim appBayar = Me.DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appbayar")
                                        Dim circulationRow = CType(Me.DgvTrnCirculationvoucherdetil.Rows(i).DataBoundItem, DataRowView).Row

                                        If appBayar.Value = False Then
                                            Dim dtRow = CType(Me.DgvTrnCirculationvoucherdetil.Rows(i).DataBoundItem, DataRowView).Row
                                            Dim circulation_id As String = dtRow.Item("circulation_id")
                                            Dim line As Integer = dtRow.Item("circulationdetil_line")
                                            Dim pv_id As String = dtRow.Item("circulationdetil_reference")
                                            Dim rekanan_id As Decimal
                                            Dim rekanan_name As String = ""
                                            Dim result As Int16

                                            db.act_TrnCirculationvoucherdetil_CheckRekananEmail(circulation_id, line, rekanan_id, rekanan_name, result)

                                            If result = 0 Or result = 2 Then
                                                Dim item As New dlgTrnCirculationRekanan.CirculationRekananItem

                                                With item
                                                    .rekanan_id = rekanan_id
                                                    .rekanan_name = rekanan_name
                                                    .pv_id = pv_id

                                                    If result = 0 Then
                                                        .deskripsi = "Karyawan tidak mempunyai email"
                                    ...
                                    Return True
                                Catch ex As Exception
                                    MsgBox("Gagal mengecek karyawan" + vbCrLf + ex.Message, MsgBoxStyle.Critical)
        ...
        Dim list As New List(Of dlgTrnCirculationRekanan.CirculationRekananItem)

        Me.Cursor = Cursors.WaitCursor
        Try
            Me._AppDirFromContext = True

            If sender Is btn_CheckAllAppDir Then
                If checkingEmail(list) = True Then
                    If list.Count > 0 Then
                        Dim dlg As New dlgTrnCirculationRekanan(list)
                ...
                For i As Integer = 0 To Me.DgvTrnCirculationvoucherdetil.Rows.Count - 1
                    If Me.DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appbayar").Value = False Then
                        Me.DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").Value = True
                    End If
                Next
            ElseIf sender Is btn_UnCheckAllAppDir Then
    ...
    Private Sub btn_List_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_List.Click
        ...
        Me.tbl_TrnCirculationvoucherdetil_all.Clear()
        For j = 0 To Me.DgvTrnCirculationvoucherdetil.Rows.Count - 1
            'tambahan
            If DgvTrnCirculationvoucherdetil.Rows(j).Cells("circulationdetil_canceled").Value <> True Then
        ...
        If retObj IsNot Nothing Then
            ...
            For i = 0 To tbl_ret.Rows.Count - 1
                row = Me.tbl_TrnCirculationvoucherdetil_all.NewRow
                ...
                row.Item("circulationdetil_descr") = Trim(tbl_ret.Rows(i).Item("rekanan_name").ToString) & " - " & tbl_ret.Rows(i).Item("jurnal_descr").ToString
                If tbl_ret.Rows(i).Item("currency_id") = "1" Then
                    row.Item("circulationdetil_amount") = tbl_ret.Rows(i).Item("jurnal_amountidr")
                    row.Item("circulationdetil_foreign") = 0 'tbl_ret.Rows(i).Item("jurnal_amountforeign") '0
                Else
                    row.Item("circulationdetil_amount") = 0 'tbl_ret.Rows(i).Item("jurnal_amountidr") '0
                ...
                Me.tbl_TrnCirculationvoucherdetil_all.Rows.Add(row)
            Next
            For i = 0 To Me.tbl_TrnCirculationvoucherdetil_all.Rows.Count - 1
                If Me.tbl_TrnCirculationvoucherdetil_all.Rows(i).RowState = DataRowState.Added Then
                    'Me.tbl_TrnCirculationvoucherdetil.NewRow()
                    Me.tbl_TrnCirculationvoucherdetil.ImportRow(tbl_TrnCirculationvoucherdetil_all.Rows(i))
                End If
            Next
            For i = 0 To Me.DgvTrnCirculationvoucherdetil.Rows.Count - 1
            ...
            Me.obj_Circulation_amount.Text = Format(totalAmountIDR, "#,##0.00")
        End If

    End Sub

    Private Sub btn_Refresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Refresh.Click
        If DetailDataPaidIsApproved() Then
            Exit Sub
        End If
    ...
    Private Sub DataIsApproved(ByVal isApproved As Boolean)
        ...
        Dim tbl_circulationVoucher_Changes_Approved As DataTable
        Dim tbl_circulationVoucherDetil_Changes_Approved As DataTable

        Me.BindingContext(Me.tbl_TrnCirculationvoucher_Temp).EndCurrentEdit()
        tbl_circulationVoucher_Changes_Approved = Me.tbl_TrnCirculationvoucher_Temp.GetChanges()
        Me.BindingContext(Me.tbl_TrnCirculationvoucher_Temp).EndCurrentEdit()

        Me.DgvTrnCirculationvoucherdetil.EndEdit()
        Me.BindingContext(Me.tbl_TrnCirculationvoucherdetil).EndCurrentEdit()
        tbl_circulationVoucherDetil_Changes_Approved = Me.tbl_TrnCirculationvoucherdetil.GetChanges()

        If tbl_circulationVoucher_Changes_Approved IsNot Nothing _
            Or tbl_circulationVoucherDetil_Changes_Approved IsNot Nothing Then
        ...
        Try
            ...
            Try
                ...
                If isApproved = True Then
                    ...
                    Me.DgvTrnCirculationvoucher.Rows(Me.DgvTrnCirculationvoucher.CurrentRow.Index).Cells("circulation_appuserby").Value = Me.UserName
                    Me.DgvTrnCirculationvoucher.Rows(Me.DgvTrnCirculationvoucher.CurrentRow.Index).Cells("circulation_appuserdt").Value = Now
                    ...
                    Me.uiTrnCirculationVoucher_FormAfterOpenRow(DgvTrnCirculationvoucher.CurrentRow)
                ...
                Else
                    ...
                    Me.DgvTrnCirculationvoucher.Rows(Me.DgvTrnCirculationvoucher.CurrentRow.Index).Cells("circulation_appuserby").Value = ""
                    Me.DgvTrnCirculationvoucher.Rows(Me.DgvTrnCirculationvoucher.CurrentRow.Index).Cells("circulation_appuserdt").Value = DBNull.Value
            ...
            dgvAllowDeleteIsDisapprove(isApproved)

            Dim rowIndeks As Integer = DgvTrnCirculationvoucher.CurrentCell.RowIndex
            Me.uiTrnCirculationVoucher_OpenRow(rowIndeks)
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
...
#Region " Layout & Init UI "
    Private Function FormatDgvTrnCirculationvoucher(ByRef objDgv As System.Windows.Forms.DataGridView) As Boolean
        ...
        Dim cCirculation_isdisabled As System.Windows.Forms.DataGridViewCheckBoxColumn = New System.Windows.Forms.DataGridViewCheckBoxColumn
        Dim cCirculation_isdisabledby As System.Windows.Forms.DataGridViewTextBoxColumn = New System.Windows.Forms.DataGridViewTextBoxColumn
        Dim cCirculation_isdisableddt As System.Windows.Forms.DataGridViewTextBoxColumn = New System.Windows.Forms.DataGridViewTextBoxColumn
        ...
        cModified_by.Name = "modify_by"
        cModified_by.HeaderText = "modify_by"
        cModified_by.DataPropertyName = "modify_by"
        cModified_by.Width = 100
        cModified_by.Visible = False
        cModified_by.ReadOnly = False

        cModified_dt.Name = "modify_dt"
        cModified_dt.HeaderText = "modify_dt"
        cModified_dt.DataPropertyName = "modify_dt"
        cModified_dt.Width = 100
        cModified_dt.Visible = False
        cModified_dt.ReadOnly = False

        cCirculation_isdisabled.Name = "circulation_isdisabled"
        cCirculation_isdisabled.HeaderText = "circulation_isdisabled"
        cCirculation_isdisabled.DataPropertyName = "circulation_isdisabled"
        cCirculation_isdisabled.Width = 80
        cCirculation_isdisabled.Visible = False
        cCirculation_isdisabled.ReadOnly = False

        cCirculation_isdisabledby.Name = "circulation_isdisabledby"
        cCirculation_isdisabledby.HeaderText = "circulation_isdisabledby"
        cCirculation_isdisabledby.DataPropertyName = "circulation_isdisabledby"
        cCirculation_isdisabledby.Width = 100
        cCirculation_isdisabledby.Visible = False
        cCirculation_isdisabledby.ReadOnly = False

        cCirculation_isdisableddt.Name = "circulation_isdisableddt"
        cCirculation_isdisableddt.HeaderText = "circulation_isdisableddt"
        cCirculation_isdisableddt.DataPropertyName = "circulation_isdisableddt"
        cCirculation_isdisableddt.Width = 100
        cCirculation_isdisableddt.Visible = False
        cCirculation_isdisableddt.ReadOnly = False
        ...
        cModified_by, cModified_dt, cCirculation_isdisabled, cCirculation_isdisabledby, cCirculation_isdisableddt})
    ...
    Private Function FormatDgvTrnCirculationvoucherdetil(ByRef objDgv As System.Windows.Forms.DataGridView) As Boolean
        ...
        cCirculationdetil_amount.DefaultCellStyle.Format = "#,##0.00"
...
#Region " User Defined Function "
    ...
    Private Function uiTrnCirculationVoucher_NewData() As Boolean
        ...
        Me.tbl_TrnCirculationvoucherdetil_all = clsDataset.CreateTblTrnCirculationvoucherdetil
        ...
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulation_id").DefaultValue = 0
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").DefaultValue = DBNull.Value
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").AutoIncrement = True
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").AutoIncrementSeed = 10
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").AutoIncrementStep = 10
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("channel_id").DefaultValue = Me._CHANNEL
    ...
    ''' <summary>
    ''' </summary>
    ''' <param name="circulation_id"></param>
    ''' <param name="circulationdetil_line"></param>
    ''' <param name="app_type">0 = Approve, 1 = Disapprove</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function uiTrnCirculationVouccher_AppDireksi(ByVal circulation_id As String,
                                                         ByVal circulationdetil_line As Integer,
                                                         ByVal app_type As Integer) As Boolean
        Dim dbConn As New OleDb.OleDbConnection(Me.DSN)
        Dim cmd As OleDb.OleDbCommand
        Dim cookies As Byte() = Nothing

        dbConn.Open()

        clsApplicationRole.SetAppRole(dbConn, cookies)

        Try
            cmd = New OleDb.OleDbCommand("act_TrnCirculationvoucherdetil_ApproveDisapproveDir", dbConn)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("@channel_id", Me._CHANNEL)
            cmd.Parameters.AddWithValue("@circulation_id", circulation_id)
            cmd.Parameters.AddWithValue("@circulationdetil_voucher", circulationdetil_line)
        ...
        Catch ex As Exception
            Throw ex
        Finally
            clsApplicationRole.UnsetAppRole(dbConn, cookies)
            dbConn.Close()
        End Try
    End Function
        ...
        If tbl_TrnCirculationvoucher_Temp_Changes IsNot Nothing Or tbl_TrnCirculationvoucherdetil_Changes IsNot Nothing Then
            Try
                ...
                If tbl_TrnCirculationvoucher_Temp_Changes IsNot Nothing Then
                    success = Me.uiTrnCirculationVoucher_SaveMaster(dbConn, dbTrans, circulation_id, tbl_TrnCirculationvoucher_Temp_Changes, MasterDataState)
                ...
                If tbl_TrnCirculationvoucherdetil_Changes IsNot Nothing Then
                    ...
                    success = Me.uiTrnCirculationVoucher_SaveDetil(dbConn, dbTrans, circulation_id, tbl_TrnCirculationvoucherdetil_Changes, MasterDataState)
                    ...
                    'success = Me.uiTrnCirculationVoucher_updateHeader(dbConn, dbTrans, circulation_id, tbl_TrnCirculationvoucherdetil_Changes, MasterDataState) pindah di approval 24/09/2018 
                    'If Not success Then Throw New Exception("Error: Save Detil Data at Me.uiTrnCirculationVoucher_SaveDetil(tbl_TrnCirculationvoucherdetil_Changes)")
                End If
    ...
    Private Function uiTrnCirculationVoucher_SaveMaster(ByVal dbConn As OleDb.OleDbConnection, ByVal dbTrans As OleDb.OleDbTransaction, ByRef circulation_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState) As Boolean
        ' Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        ...
        dbCmdInsert = New OleDb.OleDbCommand("act_TrnCirculationvoucher_Insert", dbConn, dbTrans)
        ...
        dbCmdUpdate = New OleDb.OleDbCommand("act_TrnCirculationvoucher_Update", dbConn, dbTrans)
    ...
    Private Function uiTrnCirculationVoucher_updateHeader(ByVal dbConn As OleDb.OleDbConnection, dbTrans As OleDb.OleDbTransaction, ByRef circulation_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdUpdate As OleDb.OleDbCommand
        Dim query As String
        ...
        query = String.Format("UPDATE E_FRM.dbo.transaksi_circulationvoucher SET circulation_foreign = {0},	circulation_amount = {1} WHERE circulation_id = '{2}' ",
                                  totalAmount,
                                  totalAmountIDR,
                                  clsUtil.IsDbNull(Me.DgvTrnCirculationvoucher.Rows(Me.DgvTrnCirculationvoucher.CurrentRow.Index).Cells("circulation_totalrevisi").Value, 0) + 1,
                                  Me.DgvTrnCirculationvoucher.Rows(Me.DgvTrnCirculationvoucher.CurrentRow.Index).Cells("circulation_id").Value
                                  )
        ...
        Try
            'dbConn.Open()
            'clsApplicationRole.SetAppRole(dbConn, cookie)
            dbCmdUpdate = New OleDb.OleDbCommand(query, dbConn, dbTrans)
            dbCmdUpdate.CommandType = CommandType.Text
            dbCmdUpdate.ExecuteNonQuery()
            Return True
        Catch ex As Data.OleDb.OleDbException
    ...
    Private Function uiTrnCirculationVoucher_SaveDetil(ByVal dbConn As OleDb.OleDbConnection, ByVal dbTrans As OleDb.OleDbTransaction, ByRef circulation_id As Object, ByVal objTbl As DataTable, ByVal MasterDataState As System.Data.DataRowState) As Boolean
        'Dim dbConn As OleDb.OleDbConnection = New OleDb.OleDbConnection(Me.DSN)
        Dim dbCmdInsert As OleDb.OleDbCommand
        Dim dbCmdUpdate As OleDb.OleDbCommand
        ...
        dbCmdInsert = New OleDb.OleDbCommand("act_TrnCirculationvoucherdetil_Insert", dbConn, dbTrans)
        dbCmdInsert.CommandType = CommandType.StoredProcedure
        ...
        dbCmdInsert.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_rekanan", System.Data.OleDb.OleDbType.Decimal, 8, System.Data.ParameterDirection.Input, False, CType(8, Byte), CType(0, Byte), "circulationdetil_rekanan", System.Data.DataRowVersion.Current, Nothing))
        ...
        dbCmdInsert.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_foreign", System.Data.OleDb.OleDbType.Decimal, 18, System.Data.ParameterDirection.Input, False, CType(18, Byte), CType(2, Byte), "circulationdetil_foreign", System.Data.DataRowVersion.Current, Nothing))
        dbCmdInsert.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_rate", System.Data.OleDb.OleDbType.Decimal, 18, System.Data.ParameterDirection.Input, False, CType(18, Byte), CType(2, Byte), "circulationdetil_rate", System.Data.DataRowVersion.Current, Nothing))
        dbCmdInsert.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_amount", System.Data.OleDb.OleDbType.Decimal, 18, System.Data.ParameterDirection.Input, False, CType(18, Byte), CType(2, Byte), "circulationdetil_amount", System.Data.DataRowVersion.Current, Nothing))
        dbCmdInsert.Parameters.Add(New System.Data.OleDb.OleDbParameter("@channel_id", System.Data.OleDb.OleDbType.VarWChar, 20, "channel_id"))
        ...
        dbCmdUpdate = New OleDb.OleDbCommand("act_TrnCirculationvoucherdetil_Update", dbConn, dbTrans)
        dbCmdUpdate.CommandType = CommandType.StoredProcedure
        ...
        dbCmdUpdate.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_rekanan", System.Data.OleDb.OleDbType.Decimal, 8, System.Data.ParameterDirection.Input, False, CType(8, Byte), CType(0, Byte), "circulationdetil_rekanan", System.Data.DataRowVersion.Current, Nothing))
        ...
        dbCmdUpdate.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_foreign", System.Data.OleDb.OleDbType.Decimal, 18, System.Data.ParameterDirection.Input, False, CType(18, Byte), CType(2, Byte), "circulationdetil_foreign", System.Data.DataRowVersion.Current, Nothing))
        dbCmdUpdate.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_rate", System.Data.OleDb.OleDbType.Decimal, 18, System.Data.ParameterDirection.Input, False, CType(18, Byte), CType(2, Byte), "circulationdetil_rate", System.Data.DataRowVersion.Current, Nothing))
        dbCmdUpdate.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_amount", System.Data.OleDb.OleDbType.Decimal, 18, System.Data.ParameterDirection.Input, False, CType(18, Byte), CType(2, Byte), "circulationdetil_amount", System.Data.DataRowVersion.Current, Nothing))
        dbCmdUpdate.Parameters.Add(New System.Data.OleDb.OleDbParameter("@channel_id", System.Data.OleDb.OleDbType.VarWChar, 20, "channel_id"))
        ...
        dbCmdDelete = New OleDb.OleDbCommand("act_TrnCirculationvoucherdetil_Canceled", dbConn, dbTrans)
        dbCmdDelete.CommandType = CommandType.StoredProcedure
        dbCmdDelete.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulation_id", System.Data.OleDb.OleDbType.VarWChar, 24))
        dbCmdDelete.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_line", System.Data.OleDb.OleDbType.Integer, 4, "circulationdetil_line"))
        dbCmdDelete.Parameters.Add(New System.Data.OleDb.OleDbParameter("@circulationdetil_canceledby", System.Data.OleDb.OleDbType.VarWChar, 100))
        dbCmdDelete.Parameters("@circulation_id").Value = circulation_id
        dbCmdDelete.Parameters("@circulationdetil_canceledby").Value = Me.UserName
    ...
    Private Function uiTrnCirculationVoucher_OpenRowMaster(ByVal channel_id As String, ByVal circulation_id As String, ByVal dbConn As OleDb.OleDbConnection) As Boolean
        ...
        Try
            ...
            If CType(clsUtil.IsDbNull(Me.tbl_TrnCirculationvoucher_Temp.Rows(0).Item("circulation_appuser"), 0), Integer) = 0 Then
                ...
                Me.obj_Circulation_descr.ReadOnly = False
                Me.obj_Circulation_sendto.Enabled = True
                Me.obj_Circulation_descr.BackColor = Color.White
                'tambahan
                dgvAllowDeleteIsDisapprove(False)
            Else
                Me.btnUnApproved.Visible = True
                Me.btn_List.Enabled = False
                Me.obj_Circulation_descr.ReadOnly = True
                Me.obj_Circulation_sendto.Enabled = False
                Me.obj_Circulation_descr.BackColor = Color.LightYellow
                'tambahan
                dgvAllowDeleteIsDisapprove(True)
            End If
        Catch ex As Exception
    ...
    Private Function uiTrnCirculationVoucher_OpenRowDetil(ByVal channel_id As String, ByVal circulation_id As String, ByVal dbConn As OleDb.OleDbConnection) As Boolean
        ...
        Me.tbl_TrnCirculationvoucherdetil = clsDataset.CreateTblTrnCirculationvoucherdetil()
        Me.tbl_TrnCirculationvoucherdetil_all = clsDataset.CreateTblTrnCirculationvoucherdetil
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulation_id").DefaultValue = circulation_id
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").DefaultValue = DBNull.Value
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").AutoIncrement = True
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").AutoIncrementSeed = 10
        Me.tbl_TrnCirculationvoucherdetil_all.Columns("circulationdetil_line").AutoIncrementStep = 10
        ...
        Try
            dbDA.Fill(Me.tbl_TrnCirculationvoucherdetil_all)
            If tbl_TrnCirculationvoucherdetil_all.Select("circulationdetil_canceled <> 1").Length > 0 Then
                tbl_TrnCirculationvoucherdetil = tbl_TrnCirculationvoucherdetil_all.Select("circulationdetil_canceled <> 1").CopyToDataTable
            End If
            Me.DgvTrnCirculationvoucherdetil.DataSource = Me.tbl_TrnCirculationvoucherdetil
            'Additional for exporting To Excel
            Me.DataFill(Me.tbl_trnCirculationDetil_ExportExcel, "act_TrnCirculationVoucherDetil_ExportExcel", String.Format("circulation_id='{0}'", circulation_id), Me._CHANNEL)
...
#End Region
    ...
    Private Sub uiTrnCirculationVoucher_FormAfterOpenRow(ByRef id As Object) Handles Me.FormAfterOpenRow
        For i As Integer = 0 To DgvTrnCirculationvoucherdetil.RowCount - 1
            If DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appbayar").Value = True Then
                DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").ReadOnly = True
                DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").Style.BackColor = Color.LightYellow
            Else
                DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").ReadOnly = False
                DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").Style.BackColor = Color.White
            End If
        Next
    End Sub

    Private Sub uiTrnCirculationVoucher_FormAfterSave(ByRef id As Object, result As uiBase.FormSaveResult) Handles Me.FormAfterSave
        Me.dgvAllowDeleteIsDisapprove(obj_Circulation_appuser.Checked)
        For i As Integer = 0 To DgvTrnCirculationvoucherdetil.RowCount - 1
            If DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appbayar").Value = True Then
                DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").ReadOnly = True
                DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_appdireksi").Style.BackColor = Color.LightYellow
...
#Region " Print "
    ...
    As System.IO.Stream
        Dim stream As System.IO.Stream = New System.IO.FileStream("Temp\" & name & "_" & Date.Now.Second.ToString & "." & fileNameExtension, System.IO.FileMode.Create)
...
#Region "Event On Dgv Detil"
    ...
    Private Sub DgvTrnCirculationvoucherdetil_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles DgvTrnCirculationvoucherdetil.CellValueChanged
        If e.ColumnIndex = 12 Then
            If Me.uiTrnCirculationVoucher_FormError() = True Then
                Exit Sub
            End If

            If Me._CancelCellAppDir = True Then
                Me._CancelCellAppDir = False
                CType(Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).DataBoundItem, DataRowView).Row.AcceptChanges()

                Exit Sub
            End If

            Dim currentRow As DataGridViewRow = Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex)
            Dim circulationRow = CType(currentRow.DataBoundItem, DataRowView).Row
            Dim appDir = currentRow.Cells("circulationdetil_appdireksi")

            Dim circulation_id As String
            Dim circulationdetil_line As Integer
            Dim circulationdetil_reference As String

            circulation_id = circulationRow.Item("circulation_id")
            circulationdetil_line = circulationRow.Item("circulationdetil_line")
            circulationdetil_reference = circulationRow.Item("circulationdetil_reference")

            'Jika approve lewat context, maka lewati proses ini
            '-----------------------------------------------------------------------------------------------
            If Me._AppDirFromContext = False Then
                Dim db As New DataClassesFrmDataContext(Me.DSNLinq)
                Dim rekanan_id As Decimal
                Dim rekanan_name As String = ""
                Dim result As Int16
                ...
                Try
                    db.act_TrnCirculationvoucherdetil_CheckRekananEmail(circulation_id, circulationdetil_line, rekanan_id, rekanan_name, result)

                    If result = 0 Or result = 2 Then
                        Dim item As New dlgTrnCirculationRekanan.CirculationRekananItem
                        Dim listRekanan As New List(Of dlgTrnCirculationRekanan.CirculationRekananItem)

                        With item
                            .pv_id = circulationdetil_reference
                            .rekanan_id = rekanan_id
                            .rekanan_name = rekanan_name
                        ...
                        listRekanan.Add(item)

                        Dim dlg As New dlgTrnCirculationRekanan(listRekanan)

                        If dlg.ShowDialog() = DialogResult.Cancel Then
                            Me._CancelCellAppDir = True
                            appDir.Value = Not appDir.Value

                            Exit Sub
                        End If
                    End If
                Catch ex As Exception
                    MsgBox("Gagal mengecek email." & vbCrLf & ex.Message, MsgBoxStyle.Critical)
            ...
            Try
                If currentRow.Cells(12).EditedFormattedValue = True Then
                    Me.uiTrnCirculationVouccher_AppDireksi(circulation_id, circulationdetil_line, 0)
                Else
                    Me.uiTrnCirculationVouccher_AppDireksi(circulation_id, circulationdetil_line, 1)
                End If
                ...
                End If
            Catch ex As Exception
                MessageBox.Show(ex.Message, mUiName + " - Gagal approve", MessageBoxButtons.OK, MessageBoxIcon.Error)
        ...
        ElseIf e.ColumnIndex = 21 Then
            ...
            Try
                ...
                row_error = False
                message = "Data tidak bisa dicanceled karena data sudah di jurnal"
                cell_appbayar = Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells("circulationdetil_appbayar")
                cell_cair = Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells("circulationdetil_appbankcair")
                cell_direksi = Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells("circulationdetil_appdireksi")
                ...
                If row_error Then
                    dgv_error = True
                    Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.Coral
                Else
                    Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.White
                End If

                If dgv_error Then
                    Me.tbl_TrnCirculationvoucherdetil.RejectChanges()
                    Throw New Exception("Data ada yang masih salah!")
                End If

                If Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells(21).EditedFormattedValue = True Then
                    Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells(22).Value = Me.UserName
                    Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells(23).Value = Now
                Else
                    Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells(22).Value = DBNull.Value
                    Me.DgvTrnCirculationvoucherdetil.Rows(e.RowIndex).Cells(23).Value = DBNull.Value
                End If
    ...
    Private Sub DgvTrnCirculationvoucherdetil_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles DgvTrnCirculationvoucherdetil.MouseUp
        If e.Button = Windows.Forms.MouseButtons.Right Then
            ...
            If click.Type = Windows.Forms.DataGrid.HitTestType.Cell Then
                'Me.DgvTrnCirculationvoucherdetil.CurrentCell = Me.DgvTrnCirculationvoucherdetil.Rows(click.RowIndex).Cells(click.ColumnIndex + 1)
            End If
        End If
    End Sub
#End Region

    Private Sub DgvTrnCirculationvoucherdetil_CurrentCellDirtyStateChanged(sender As Object, e As EventArgs) Handles DgvTrnCirculationvoucherdetil.CurrentCellDirtyStateChanged
        If Me.DgvTrnCirculationvoucherdetil.IsCurrentCellDirty Then
            Me.DgvTrnCirculationvoucherdetil.CommitEdit(DataGridViewDataErrorContexts.Commit)
        End If
    End Sub

    Private Function dgvAllowDeleteIsDisapprove(ByVal approve As Boolean) As Boolean
        If approve Then
            DgvTrnCirculationvoucherdetil.AllowUserToDeleteRows = False
        Else
            DgvTrnCirculationvoucherdetil.AllowUserToDeleteRows = True
        End If
    End Function

    Private Sub DgvTrnCirculationvoucherdetil_UserDeletedRow(sender As Object, e As DataGridViewRowEventArgs) Handles DgvTrnCirculationvoucherdetil.UserDeletedRow
        Dim totalAmountForeign, totalAmountIdr As Decimal
        For i As Integer = 0 To Me.DgvTrnCirculationvoucherdetil.Rows.Count - 1
            totalAmountForeign += clsUtil.IsDbNull(Me.DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_foreign").Value, 0)
            totalAmountIdr += clsUtil.IsDbNull(Me.DgvTrnCirculationvoucherdetil.Rows(i).Cells("circulationdetil_amount").Value, 0)
        Next
        Me.obj_Circulation_amount.Text = String.Format("{0:#,##0.00}", totalAmountIdr)
        Me.obj_Circulation_foreign.Text = String.Format("{0:#,##0.00}", totalAmountForeign)
    End Sub

    Private Function DetailDataPaidIsApproved() As Boolean

        For i As Integer = 0 To Me.tbl_TrnCirculationvoucherdetil.Rows.Count - 1
            If Me.tbl_TrnCirculationvoucherdetil.Rows(i).Item("circulationdetil_appbayar") = True Then
                Return True
            End If
        Next
    ...
    Private Sub CancelThisRowToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CancelThisRowToolStripMenuItem.Click
        If Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceled").Value = False Then
            Dim bayar As Boolean = Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_appbayar").Value
            Dim dir As Boolean = Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_appdireksi").Value

            If bayar = True Then
                MsgBox("Maaf sudah di bayar")
            Else
                If dir = True Then
                    MsgBox("Maaf sudah di approve direksi")
                Else
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceled").Value = True
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceledby").Value = Me.UserName
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceleddt").Value = Date.Now()
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.DefaultCellStyle.BackColor = Color.Beige
                End If
            End If
        End If
    End Sub

    Private Sub UncancelThisRowToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles UncancelThisRowToolStripMenuItem.Click
        If Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceled").Value = True Then
            Dim bayar As Boolean = Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_appbayar").Value
            Dim dir As Boolean = Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_appdireksi").Value
            ...
            Else
                If dir = True Then
                    MsgBox("Maaf sudah di approve direksi")
                Else
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceled").Value = False
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceledby").Value = DBNull.Value
                    Me.DgvTrnCirculationvoucherdetil.CurrentRow.Cells("circulationdetil_canceleddt").Value = DBNull.Value
                End If
            End If
        End If
    End Sub
