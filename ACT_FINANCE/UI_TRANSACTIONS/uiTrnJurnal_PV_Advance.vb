...
Public Class uiTrnJurnal_PV_Advance
    ...
    Private tbl_TrnBankentrydetil As DataTable = clsDataset.CreateTblTrnBankentrydetilReference()
...
#Region " Window Parameter "
    ...
    'Private _USER_TYPE As String = "SPV" '"STAFF" ' 
    Private _USER_TYPE As USER_TYPE = USER_TYPE.STAFF
#End Region
...
#Region " Additional Overrides "
    ...
    Private Sub btnPost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPost.Click, btnUnPost.Click
        ...
        If obj.Name = "btnPost" Then
            ...
            ' cek balance posting
            If Me._USER_TYPE = USER_TYPE.SPV Then
...
#Region " Dialoged Control "
    Private Function DialogOpen_Reference_Advance() As Boolean
        Dim dlg As dlgTrnJurnal_PV_Select_Advance = New dlgTrnJurnal_PV_Select_Advance(Me.DSN, Me.tbl_MstRekananGrid.Copy, Me._SOURCE)
...
#End Region
    ...
    Private Sub uiTrnJurnal_PV_Advance_FormAfterOpenRow(ByRef id As Object) Handles Me.FormAfterOpenRow
        ...
        '============ LOCKINGAN 20160509 =======================
        Dim objRow As DataRow = CType(Me.BindingContext(Me.tbl_TrnJurnal_Temp).Current, DataRowView).Row
        Dim jurnal_isdisabled As Boolean = objRow.Item("jurnal_isdisabled")
        Dim jurnal_isposted As Boolean = objRow.Item("jurnal_isposted")

        If jurnal_isdisabled = True Then
            Me.tbtnSave.Enabled = False
            Me.tbtnDel.Enabled = False
        ...
        Else
            If Me._USER_TYPE = USER_TYPE.STAFF And jurnal_isposted = True Then
                Me.tbtnSave.Enabled = False
                Me.tbtnDel.Enabled = False
            Else
                locking.TryLocking(id)
            End If
        End If
        '===================================================================================
    ...
    Private Sub uiTrnJurnal_PV_Advance_FormBeforeNew() Handles Me.FormBeforeNew
        ...
        '============ LOCKINGAN 20160509 =======================
        If Me.locking IsNot Nothing Then
            Me.locking.Clear()
        End If
        '=======================================================
    ...
    '============ LOCKINGAN 20160509 =======================
    Private Sub uiTrnJurnal_PV_Advance_FormAfterDelete() Handles Me.FormAfterDelete
        If Me.locking IsNot Nothing Then
            Me.locking.Clear()
        End If
    End Sub
    ...
    Public Sub Form_Load(ByVal sender As Object)
        ...
        If Me.Browser IsNot Nothing Then
            ...
            Dim uType As String = Me.GetValueFromParameter(objParameters, "USERTYPE")

            If uType.ToLower() = "staff" Then
                Me._USER_TYPE = USER_TYPE.STAFF
            ElseIf uType.ToLower() = "spv" Then
                Me._USER_TYPE = USER_TYPE.SPV
            End If
        End If
...
#Region "Event On Dgv Credit"
    ...
    Private Sub DgvTrnJurnaldetil_Credit_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles DgvTrnJurnaldetil_Credit.MouseUp
        If e.Button = Windows.Forms.MouseButtons.Right Then
            ...
            If click.Type = Windows.Forms.DataGrid.HitTestType.Cell Then
                ContextMenuStripCredit.Visible = True
            ...
            If Me.ftabMain.SelectedIndex = 0 Then
                For i = 0 To Me.DgvTrnJurnal.SelectedRows.Count - 1
                    ...
                    Else
                        ...
                        'objReportH.ReportEmbeddedResource = "ACT_FINANCE.rptPV_HeaderListAdvance1.rdlc"
                    End If
...
#Region " Untuk BackgroundWorker"
    ...
    Private Sub uiTrnJurnal_PV_Advance_CollectionData_with_BackgroundWorker(ByVal worker As BackgroundWorker)
        ...
        Dim dt_accref7 As DataTable = New DataTable
        ...
        Me.DataFill(dt_accref7, "act_selectAccRef_Detil", "accref_id = 9016") 'akun8001000
        ...
        akun1111910 = dt_accref7.Rows(0).Item(0)
...
#End Region
        ...
        If obj_Jurnal_isposted.CheckState = CheckState.Unchecked Then
            If dgv.CurrentRow.Cells("branch_id").Value = 0 Then 'branch_id (TANDA UNTUK UBAH DARI RATE TAX ATAU AMOUNT IDR TAX)
                If dgv.CurrentRow.Cells("acc_id").Value <> akun8500015 And dgv.CurrentRow.Cells("acc_id").Value <> akun8002000 And _
                    ...
                    dgv.CurrentRow.Cells("acc_id").Value <> akun8500011 And dgv.CurrentRow.Cells("acc_id").Value <> akun8001000 And _
                    dgv.CurrentRow.Cells("acc_id").Value <> akun1111910 Then
    ...
    Private Sub AddInvoiceToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AddInvoice.Click
        'pv_advance ini belum dipakai
    ...
    Private Sub ChangeAmountIDRToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ChangeAmountIDR.Click
        Dim idr As String
        Dim row As DataRow
        Dim selisih_idr, jumlah_idr, selisih_foreign, jumlah_foreign As Decimal
        Dim amountChangeSelisih As Decimal = 0
        ...
        If amountSelisih > 0 Then
            If rowCredit = 0 Then
                If rowDebit > 0 Then
                    For i = 0 To Me.tbl_TrnJurnaldetil_Debit.Rows.Count - 1
                        If Me.tbl_TrnJurnaldetil_Debit.Rows(i).RowState <> DataRowState.Deleted Then
                            If clsUtil.IsDbNull(Me.tbl_TrnJurnaldetil_Debit.Rows(i).Item("acc_id"), 0) = akun3 Then
                                Me.tbl_TrnJurnaldetil_Debit.Rows(i).Delete()
                                Exit For
                            End If
                        End If
    ...
    Private Sub ChangeAmountIDRTaxToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ChangeAmountIDRTax.Click
    ...
    Public Sub SetControls_LockingTransactionTryLocking() Implements ILocking.SetControls_LockingTransactionTryLocking
        ...
        If Me._USER_TYPE = USER_TYPE.STAFF Then
            Select Case status
                ...
                Case clsLockingTransaction.LockStatus.LockedByMe
                    ...
                    End If
            End Select
        ElseIf Me._USER_TYPE = USER_TYPE.SPV Then
            Select Case status
                Case clsLockingTransaction.LockStatus.Locked
                    Me.tbtnDel.Enabled = False
                    Me.tbtnSave.Enabled = False
        ...
        'Dim currentRow As DataRow = CType(Me.BindingContext(Me.tbl_TrnJurnal_Temp).Current, DataRowView).Row
        'Dim jurnal_isposted As Boolean = currentRow.Item("jurnal_isposted")

        'Select Case locking.Status
        '    Case clsLockingTransaction.LockStatus.LockedByMe
        '        If jurnal_isposted = True Then
        '            Me.tbtnSave.Enabled = False
        '            Me.tbtnDel.Enabled = False
        '        Else
        '            Me.tbtnSave.Enabled = True
        ...
        '        Me.ContextMenuStripCredit.Enabled = True
        '        Me.ContextMenuStripDebit.Enabled = True 'Me.ContextMenuStrip2.Enabled = True

        '        If btnPost.Visible = True Then
        '            btnPost.Enabled = True
        '        ElseIf btnUnPost.Visible = True Then
        ...
        '        Me.tbtnPrint.Enabled = True
        '        Me.tbtnPrintPreview.Enabled = True
        '        Me.ContextMenuStripCredit.Enabled = False
        '        Me.ContextMenuStripDebit.Enabled = False 'Me.ContextMenuStrip2.Enabled = False
    ...
    Private Sub btn_feeRate2_Click(sender As Object, e As EventArgs) Handles btn_feeRate2.Click
        Dim rate As String
        Dim row As DataRow
        Dim selisih_idr, jumlah_idr, selisih_foreign, jumlah_foreign As Decimal
        Dim amountChangeSelisih As Decimal = 0
    ...
    Private Sub ChangeRateTaxToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ChangeRateTaxToolStripMenuItem.Click
        Dim rate As String
        Dim row As DataRow
        Dim selisih_idr, jumlah_idr, selisih_foreign, jumlah_foreign As Decimal
        Dim amountChangeSelisih As Decimal = 0
